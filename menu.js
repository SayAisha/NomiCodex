...
export class PageManager {
    constructor() {
        ...
        if (this.pageListContainer === null) {
            console.log("Calculator UI absent (NEI-only mode), skipping PageManager");
            this.pageListContainer = document.createElement('div');
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('#nei-link, #nei-link-2').forEach((el) => {
    ...
    switchPage(pageName) {
        ...
        if (page) {
            SetCurrentPage(page);
            // Update window title
            document.title = `${page.name} - NomiFactory CEu calculator`;
        }
        this.render();
    }
